...
class QmrClauApp:
    ...
    def _check_for_updates(self):
        """Comprova en segon pla si hi ha una versió nova a GitHub."""
        def _fetch():
            try:
                url = f"https://api.github.com/repos/{GITHUB_REPO}/releases/latest"
                req = urllib.request.Request(url, headers={"User-Agent": "qmrClau"})
                ...
                pass  # Sense connexió o error → ignora silenciosament

        t = threading.Thread(target=_fetch, daemon=True)
        t.start()

    def _show_update_banner(self, new_version, url):
        """Mostra un banner discret a la part superior quan hi ha actualització."""
        if not hasattr(self, "content"):
            return
        banner = tk.Frame(self.root, bg="#2d4a22", pady=4)
        # Insereix el banner just després de la toolbar (primer fill de root)
        children = self.root.pack_slaves()
        banner.pack(fill="x", after=children[0] if children else None)
        msg = tk.Label(banner,
                       text=f"🆕  Nova versió disponible: {new_version}  (actual: v{APP_VERSION})",
                       font=("Segoe UI", 9), fg="#a8d8a8", bg="#2d4a22")
        msg.pack(side="left", padx=(12, 8))
        def _download():
            webbrowser.open(url)
        btn_dl = self._make_small_button(banner, "⬇ Baixar", _download, "#3a6b2a")
        btn_dl.pack(side="left", padx=4)
        btn_close = self._make_small_button(banner, "✕", banner.destroy, "#2d4a22")
